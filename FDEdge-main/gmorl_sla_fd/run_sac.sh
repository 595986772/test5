#!/usr/bin/env bash
# 路②: 扩散-SAC 抗塌缩 (跨样本支撑多样性 + 自动调温) vs 旧塌缩扩散 vs 高斯。
# 两档 div_target {0.05, 0.10}; 每档 train -> eval(m1/m32) -> 诊断(支撑塌缩 + critic对账)。
set +e
cd "D:/python_project/实验版5/FDEdge-main/gmorl_sla_fd" || exit 1
LOG=result2/frac/run_sac_master.log; : > "$LOG"
...
say "=== 路②扩散-SAC: 抗塌缩多样性 (div_target 0.05 / 0.10) ==="
for DTGT in 005 010; do
  case $DTGT in 005) V=0.05;; 010) V=0.10;; esac
  TAG=seq_het_sac${DTGT}
