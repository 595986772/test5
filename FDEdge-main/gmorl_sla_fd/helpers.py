...
def hypervolume_2d(points, ref):
    ...
    points: (N,2)；ref: (r_x,r_y)

    只统计**严格支配 ref** 的点 (两轴都优于 ref)。落在 ref 框外的点按 HV 定义
    贡献 0, 必须先滤掉 —— 否则 x>=ref[0] 的点仍会通过「相邻点宽度 x_next-x_i」
    累加出虚假面积 (旧 bug: 让高 delay/低 energy 的极端点凭空灌水, 例如
    greedy_energy delay~444>>ref 却得 HV≈291)。滤点后 sweep 的最后一段宽度
    用 ref[0] 收口才成立。
    """
