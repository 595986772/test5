"""
edge_frequency_sweep.py
========================
测试边缘服务器不同处理能力对延迟的影响。保持云端频率不变 (50,70) GHz，
将边缘频率分为 5 档，以当前默认值 (10,40) GHz 的中点 25 GHz 为中心，
从高到低每档独立完成训练+评估，最终输出延迟-频率曲线图。

5 档 (高→低): 45 / 35 / 25 / 15 / 5 GHz (单点固定, 非范围)
输出: result_frequency/edge_frequency_sweep.png + edge_sweep_summary.csv
...
def _make_adapter(edge_f_val):
    """为固定 edge_f 创建 V2 适配器, cloud 保持默认范围."""
    ...
    class _Adapter(MOFDEnvironmentV2):
        def __init__(self, Emax=7, num_tasks_max=10,
            ...
            super().__init__(
                ...
                f_range=(float(edge_f_val), float(edge_f_val)),
...
def run_level(f_val, label):
    """训练+评估单档边缘频率, 返回 (avg_delay, avg_energy, delays, energies)."""
    prefix = f'edge_f{f_val}'
    print(f'\n{"=" * 60}')
    print(f'  Edge Sweep: {label}  |  prefix = {prefix}')
    print(f'{"=" * 60}')

    _mm.MOFDEnvironment = _make_adapter(f_val)

    cfg = _base_cfg(prefix)
    ...
    print(f'  [{label}] avg_delay={avg_d:.3f}s  avg_energy={avg_e:.3f}J  '
          f'min_delay={delays.min():.3f}s  max_delay={delays.max():.3f}s')
...
def plot(results, output_dir):
    ...
    all_d = np.array([r['delays'] for r in results])
    ...
    ax.set_xlabel('Edge CPU Frequency (GHz)', fontsize=12)
    ax.set_ylabel('Average Delay (s)', fontsize=12)
    ax.set_title('Edge Frequency → Task Delay', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=9)
    ax.invert_xaxis()  # 高频率在左, 符合"从高到低"
    ...
    ax.set_title('Edge Frequency → Task Energy', fontsize=14, fontweight='bold')
    ax.grid(True, alpha=0.25)
    ax.legend(fontsize=9)
    ax.invert_xaxis()
    ...
    fig.suptitle('Impact of Edge Processing Capability on Task Offloading Performance',
                 fontsize=15, fontweight='bold', y=1.01)
    plt.tight_layout()

    save_path = os.path.join(output_dir, 'edge_frequency_sweep.png')
...
def main():
    print(f'Edge Frequency Sweep: {len(EDGE_F_LEVELS)} levels × {EPOCHS} epochs')
    print(f'Output dir: {OUTPUT_DIR}')
    print(f'Est. time: ~{len(EDGE_F_LEVELS) * EPOCHS * 3}-{len(EDGE_F_LEVELS) * EPOCHS * 5} min')

    os.makedirs(OUTPUT_DIR, exist_ok=True)

    results = []
    for f_val, label in EDGE_F_LEVELS:
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'edge_sweep_summary.csv')
    with open(csv_path, 'w', encoding='utf-8') as f:
        f.write('edge_f_ghz,label,avg_delay_s,avg_energy_j\n')
        for r in results:
