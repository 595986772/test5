"""
Discrete-SAC baseline V2 (on MOFDEnvironmentV2 with cloud action)
=================================================================
对齐 GMORL 论文 Pareto 协议:
  - action ∈ [0, Emax]  (action 0 = cloud, 1..Emax = edges)
  - num_tasks_max = 10  (V2 默认, 避免 cloud 单服务器队列爆)
  - 其余训练/评估流程与 V1 (dsac_main.py) 完全一致
输出: results/dsac_v2_pareto_aggregated.csv 等
"""
import os
import sys
import time
...
def run_single_seed(cfg, seed, device):
    ...
    print(f"[dsac-v2 seed {seed}] fixed HV ref = ({fixed_ref[0]:.3f}, {fixed_ref[1]:.3f})")
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        if (epoch + 1) % log_interval == 0 or epoch == 0:
            ...
            print(f"[dsac-v2 seed {seed} epoch {epoch + 1:03d}/{cfg['num_epochs']}] "
...
def main():
    cfg = dict(
        ...
        # ---- Discrete-SAC 超参 (与 V1 保持一致, 便于纯环境差异对比) ----
        alpha_T=1.0, alpha_E=0.25,
    ...
    print('[dsac-v2] Config:', cfg)
    os.makedirs(RESULTS_DIR, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('[dsac-v2] Device:', device)
    ...
    print(f"\n[dsac-v2] all seeds done in {time.time() - t_all:.1f}s")
    ...
    np.savetxt(os.path.join(RESULTS_DIR, 'dsac_v2_hv_all_seeds.csv'), hv_all, fmt='%.5f')
    np.savetxt(os.path.join(RESULTS_DIR, 'dsac_v2_hv_mean_std.csv'),
               np.stack([hv_m, hv_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, 'dsac_v2_avg_delay_mean_std.csv'),
               np.stack([d_m, d_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, 'dsac_v2_avg_energy_mean_std.csv'),
               np.stack([e_m, e_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, 'dsac_v2_pareto_aggregated.csv'), pts_pool, fmt='%.5f')
    ...
    for ax, title, mu, sd, color in zip(
        ...
        ax.set_title(f'[Discrete-SAC-v2] {title} ({len(all_runs)} seeds, smooth={w})')
        ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, 'dsac_v2_training_curves.png'), dpi=150)
    plt.close()

    ref = (float(pts_pool[:, 0].max() * 1.1 + 1e-6),
           float(pts_pool[:, 1].max() * 1.1 + 1e-6))
    hvs = [hypervolume_2d(r['pts'], ref) for r in all_runs]
    print(f'\n[dsac-v2] Final HV (ref={ref}): mean={np.mean(hvs):.4f} std={np.std(hvs):.4f}')

    with open(os.path.join(RESULTS_DIR, 'dsac_v2_summary.txt'), 'w', encoding='utf-8') as f:
        f.write('=== Discrete-SAC-v2 Multi-seed Summary ===\n')
    ...
    print(f'[dsac-v2] Done. Results in {RESULTS_DIR}')
