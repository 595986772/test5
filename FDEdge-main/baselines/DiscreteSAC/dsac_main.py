"""
Discrete-SAC baseline main (同 MOFD 环境, 同 5-seed 协议).
直接与 mofd_main.py 输出文件对齐: results/dsac_pareto_aggregated.csv 等,
方便与 G-FDEdge 共同绘制 Pareto / HV 对比图.
"""
import os
import sys
import time
...
# 把项目根目录加入 sys.path, 便于导入 mofd_environment / helpers / mofd_main
...
from dsac_model import DiscreteSAC, ReplayBuffer

RESULTS_DIR = os.path.join(PROJECT_ROOT, 'results')


class RewardNormalizer:
    """逐目标奖励归一化 (PopArt-lite, 对齐 mofd_v5 PC-FDN).
...
def run_single_seed(cfg, seed, device):
    ...
    print(f"[dsac seed {seed}] fixed HV ref = ({fixed_ref[0]:.3f}, {fixed_ref[1]:.3f})")
    ...
    for epoch in range(cfg['num_epochs']):
        ...
        for _ in range(cfg['n_prefs_per_epoch']):
            ...
            ep_ds.append(d); ep_es.append(e); ep_hs.extend(losses)
    ...
    if normalizer is not None:
        ...
        print(f"[dsac seed {seed}] learned reward σ: "
              f"σ_T={sig[0]:.4f} σ_E={sig[1]:.4f} (ratio σ_T/σ_E={sig[0] / sig[1]:.3f})")
...
def main():
    cfg = dict(
        ...
        # ---- 逐目标奖励归一化 (PopArt-lite, 对齐 PC-FDN; 写到 dsac_norm_*) ----
        use_reward_norm=True, reward_norm_beta=0.01, file_tag=None,
        # ---- Discrete-SAC 超参 ----
    ...
    tag = cfg.get('file_tag') or ('dsac_norm' if cfg.get('use_reward_norm') else 'dsac')
    print(f'[dsac] Config (output tag = {tag}):', cfg)
    os.makedirs(RESULTS_DIR, exist_ok=True)
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print('[dsac] Device:', device)
    ...
    print(f"\n[dsac] all seeds done in {time.time() - t_all:.1f}s")
    ...
    np.savetxt(os.path.join(RESULTS_DIR, f'{tag}_hv_all_seeds.csv'), hv_all, fmt='%.5f')
    np.savetxt(os.path.join(RESULTS_DIR, f'{tag}_hv_mean_std.csv'),
               np.stack([hv_m, hv_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, f'{tag}_avg_delay_mean_std.csv'),
               np.stack([d_m, d_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, f'{tag}_avg_energy_mean_std.csv'),
               np.stack([e_m, e_s], axis=1), fmt='%.5f',
               header='mean_smooth std_smooth', comments='')
    np.savetxt(os.path.join(RESULTS_DIR, f'{tag}_pareto_aggregated.csv'), pts_pool, fmt='%.5f')
    ...
    for ax, title, mu, sd, color in zip(
        ...
        ax.set_title(f'[Discrete-SAC] {title} ({len(all_runs)} seeds, smooth={w})')
        ax.grid(alpha=0.3)
    plt.tight_layout()
    plt.savefig(os.path.join(RESULTS_DIR, f'{tag}_training_curves.png'), dpi=150)
    plt.close()

    ref = (float(pts_pool[:, 0].max() * 1.1 + 1e-6),
           float(pts_pool[:, 1].max() * 1.1 + 1e-6))
    hvs = [hypervolume_2d(r['pts'], ref) for r in all_runs]
    print(f'\n[dsac] Final HV (ref={ref}): mean={np.mean(hvs):.4f} std={np.std(hvs):.4f}')

    with open(os.path.join(RESULTS_DIR, f'{tag}_summary.txt'), 'w', encoding='utf-8') as f:
        f.write('=== Discrete-SAC Multi-seed Summary ===\n')
    ...
    print(f'[dsac] Done. Results in {RESULTS_DIR}')
